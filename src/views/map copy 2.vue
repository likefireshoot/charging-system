<template>
  ...
  <div class="cardBody">
   ...
   <div class="detailCard">
      ...
      <div class="item"><h2>位置：</h2><p>{{data.address}}</p></div>
      <div v-if="data.exception==0" class="item"><h2>状态：</h2><img v-if="!data.status=='开'" src="@/assets/fault/close.png" /><p><p>{{data.status=="开"?'开启':'关阀'}}</p></div>
      <div v-if="data.exception==1" class="item"><h2>状态：</h2><img src="@/assets/fault/icon5.png" /><p>异常</p></div>
      <div v-if="data.pointTypes==8" class="item"><h2>电压：{{data.externInfo.voltage || 0}}V</h2></div>
      <div v-if="data.pointTypes==8" class="item"><h2>电流：{{data.externInfo.electricity || 0}}A</h2></div>
      <div v-if="data.pointTypes==0" class="item"><h2>压力：{{data.externInfo.voltage}}MPa</h2></div>
    </div> 
...
function setMap(item) {
  ...
  if (item.id == 0) {
    aMaps.setLayers([new AMap.Buildings({
      zIndex: 10,
...
function setPolyline(item) {
  ...
  item.map(a => {
    ...
    polylines.push(new AMap.Polyline(PolylineItem));
  });
  aMaps.add(polylines);
}
