  ...
  <div class="cardBody over">
    ...
    <div class="setCard">
      ...
      <div class="timeTxt">
        ...
        <p>当前轨迹时间</p>
        <h2>{{nowTime?formatHourMinute(new Date(nowTime)):'00:00'}}</h2>
      </div>
    </div>
...
async function getTrack() {
  ...
  let UserTrack = data.reduce((list, item) => {
    return [...list, ...item.points];
  }, [])


  frametList = splitArray(UserTrack, 18);


  if (frametList.length > 0) {

    frametList.unshift(frametList[0].filter((a, index) => index == 0));
    let last = frametList[frametList.length - 1].filter((a, index) => index >= frametList[frametList.length - 1].length - 1);
    frametList.push(last);
    setMarketList(false);
    aMaps.panTo([frametList[0][0].longitude, frametList[0][0].latitude], 250);
...
function play() {
  ...
  if (playType.value == 0) return;
  let playframe = frameNum.value;
  let length = frametList.length;


  if (playType.value == 1) {
    if (playframe < length - 1) {
      frameNum.value = playframe + 1;
      setMarketList(false);
      setTimeout(() => {
        play();
      }, playMultiple.value * 1000)
      return;
    }
...
function RunPlay() {
  ...
  if (isPlayIng.value && playType.value == 1) {
    ElMessage.warning('当前已在播放');
    return;
  };
  pause();
...
function rePlay() {
  if (isPlayIng.value && playType.value == 2) {
    ElMessage.warning('当前已在回放');
    return;
  };
  pause();
  setTimeout(() => {
...
function setMultiple(index) {
  ...
  let Multiple = playMultiple.value;

  console.log(Multiple);

  //快退
  if (index == 1) {
    if (playMultiple.value >= 2.75) {
      ElMessage.warning('已达到最大倍数！');
      return;
    }
    playMultiple.value = parseFloat(Number(Multiple) + 0.25).toFixed(2);
  }
  //快进
  if (index == 2) {
    if (playMultiple.value <= 0.25) {
...
function nowMarket(){
  ...
  let market=circleMarkers.find((a,index)=>index==circleMarkers.length-1);
  ...
  let lntlat=market.getExtData();
  aMaps.panTo([lntlat.longitude, lntlat.latitude], 250);
