            ...
            <div class="set-content">
              ...
              <div v-else>
                ...
                <div class="set-content-container" v-if="node.label === '设置抄表配置'">
                  <div class="set-input">
                    ...
                    <el-select v-model="params_set_tai.chaobiaoPeizhi.way">
                      <el-option label="历史上传" value="历史上传"></el-option>
                      <el-option label="实时上传" value="实时上传"></el-option>
                    </el-select>
                  </div>
                  <div class="set-input" style="width: 100%; flex-direction: row; justify-content: flex-start">
                    ...
                    <div style="width: 24%; display: flex; flex-direction: column; margin-right: 15px">
                      <span style="font-size: 14px; color: #747374; margin-bottom: 5px">抄数间隔(天)</span>
                      <el-input style="height: 35px; width: 100%" v-model="params_set_tai.chaobiaoPeizhi.uploadDate"></el-input>
                    </div>
                    <div style="width: 24%; display: flex; flex-direction: column; margin-right: 15px">
                      <span style="font-size: 14px; color: #747374; margin-bottom: 5px">抄数间隔(小时)</span>
                ...
                <div class="set-content-container" v-if="node.label === '设置水表配置'">
                  <div class="set-input">
                    <span>倍率</span>
                    <el-input v-model="params_set_tai.beilv"></el-input>
                  </div>
                </div>
  ...
  methods: {
    ...
    commit_taiyangneng() {
      if (this.node.label === "设置心跳上报间隔") {
        ...
        const formData = new FormData();
        const imei = this.data.imei;
        const uploadIntervalType = this.params_set_tai.chaobiaoPeizhi.way;
        const hourInterval = parseInt(this.params_set_tai.chaobiaoPeizhi.chaobiaoHour, 10);
        const minuteInterval = parseInt(this.params_set_tai.chaobiaoPeizhi.chaobiaoMinute, 10);
        const dayInterval = parseInt(this.params_set_tai.chaobiaoPeizhi.uploadDate, 10);
        const hourIntervalUpload = parseInt(this.params_set_tai.chaobiaoPeizhi.uploadHour, 10);
        // 将表单数据添加到FormData
        formData.append("imei", imei);
        formData.append("upload_interval_mode", uploadIntervalType);
        formData.append("interval_hour", hourInterval);
        formData.append("interval_minute", minuteInterval);
        formData.append("upload_day", dayInterval);
        formData.append("upload_hour", hourIntervalUpload);
        service
          .post("/solarEnergy/waterMeterSetReadConfig86H", formData)
          .then((response) => {
            if (response.code === 200) {
        ...
        const formData = new FormData();
        const imei = this.data.imei;
        const meterReading = parseInt(this.params_set_tai.count, 10);
        formData.append("imei", imei);
        formData.append("read_count", meterReading);
        service
          .post("/solarEnergy/waterMeterSetReadCount96H", formData)
          .then((response) => {
            if (response.code === 200) {
        ...
        const formData = new FormData();
        const imei = this.data.imei;
        const statusValue = parseInt(this.params_set_tai.famenstate, 10);
        formData.append("imei", imei);
        formData.append("status", statusValue);
        service
          .post("/solarEnergy/waterMeterSetOpenClose87H", formData)
          .then((response) => {
            if (response.code === 200) {
