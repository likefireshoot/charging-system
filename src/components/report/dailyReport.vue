    ...
    <div class="baobiao-info">
      ...
      <div class="week-report">
        <div class="week-report-title">
          <span style="font-size: 16px; margin-top: 10px; margin-bottom: 5px">日报表统计（{{ params.record_time }}）</span>
...
export default {
  ...
  methods: {
    ...
    getTradeData() {
      ...
      service
        .get(`/dayReport?record_time=${this.params.record_time}&region=${this.params.region}`)
        .then((response) => {
          if (response.code === 200) {
    ...
    getWeekTradeData() {
      ...
      service
        .get(`/weekReport?startTime=${formattedStartTime}&endTime=${this.params.record_time}&region=${region}`)
        .then((response) => {
          if (response.code === 200) {
